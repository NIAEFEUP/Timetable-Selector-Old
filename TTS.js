...
// Campos do horário
var COPY_LINK_CLIPBOARD_DEFAULT_TEXT = "Copiar link";
...
function Aula(jsonobj) {
  ...
  this.txthora =
    "" +
    ((this.horarow + this.horarow % 2) / 2 + 7) +
    ":" +
    ((this.horarow - 1) % 2) * 3 +
    "0";
  this.id = aulaid;
...
Cadeira.prototype.selectorhtml = function() {
  ...
  for (var i = 0; i < this.praticas.length; i++) {
    if (!this.praticas[i].repetido) {
      str +=
        '<option value="' +
        this.praticas[i].turma +
        '">' +
        this.praticas[i].selecttext();
      ...
      if (this.praticas[i].vagas != "?")
        str += " (" + this.praticas[i].vagas + ")";
      str += "</option>";
    }
  }
  ...
  if (this.praticas.length == 0)
    str += '<option value="teoricas">só teoricas</option>';
  str += "</select>";
  ...
  str +=
    '<img class="selectorwarning" data-cadeira="' +
    this.nome +
    '"src="error.png" alt="conflito" title="conflito">';
  //str+='</div>';
  str += "</div>";
...
Aula.prototype.horariohtml = function() {
  ...
  str +=
    ...
    "px;width:" +
    this.stwidth +
    'px;"><div class="aulawrapper">';
  str +=
    '<p class="aula"><span class="aula"><abbr title="' +
    this.cadeiran +
    '">' +
    this.cadeira +
    '</abbr></span><span class="aula"><abbr title="' +
    this.prof +
    '">' +
    this.profsig +
    "</abbr></span></p>";
  str +=
    '<p class="aula"><span class="aula">' +
    this.sala +
    '</span><span class="aula">' +
    this.turmac +
    "</span></p>";
  str += "</div></div>";
  return str;
      ...
      for (i = 0; i < aulaid; i++) {
        if (aulas[i] == true && j != i) {
          ...
          if (
            ...
            $(
              '.selectorwarning[data-cadeira="' + divaula.data("cadeira") + '"]'
            ).addClass("selectorwarningoverlap");
...
$(document).ready(function() {
  ...
  if (!window.location.hash) {
    ...
    var semestre = 1;
    if (mm < 8) {
      //janeiro a julho selecionar 2º semestre, agosto a dezembro 1º
      ano_l = ano_l - 1;
      semestre = 2;
    }
    $("#anoselect").val(ano_l);
    //$('#anoselect').prop('disabled', true);
    $("#semestreselect option")
      .filter("[value=" + semestre + "]")
      .prop("selected", true);
  ...
  $(document).on("change", ".classselector select.turmaselect", function(
    ...
    cadeiras[this.getAttribute("data-cadeira")].turmaselect = this.options[
      this.selectedIndex
    ].value;
  ...
  $(document).on("click", "input.listcadano", function(event) {
    ...
    //alert(ano);
    if (checked == true)
      $('input.listcad[data-ano="' + ano + '"]').prop("checked", true);
